...
<link rel="stylesheet" href="<?php echo plugins_url(); ?>/<?php echo $plugin_dir; ?>/css/autocomplete.css">
<script src='<?php echo plugins_url(); ?>/<?php echo $plugin_dir; ?>/js/autocomplete.js'></script>
<script src='<?php echo plugins_url(); ?>/<?php echo $plugin_dir; ?>/js/dimensions.js'></script>
...
.coloring span{float:left;}
    ...
    <script>
        $(function() {
			$('#wp_netries_options').submit(function() {
				 ...
				 $.ajax({
				  ...
				  data: "agent="+agent+"&property="+property+"&city="+city+"&county="+country,
				}).done(function(res) {  
					// alert(res);
					$('.list-wrap').html(res);
				});
			...
			$("#country").focus(function(){
				setAutoComplete("country", "results", "<?php echo plugins_url(); ?>/<?php echo $plugin_dir; ?>/array.php?key=cntry&part=");
			});
			$("#city").focus(function(){
				setAutoCompletecity("city", "results", "<?php echo plugins_url(); ?>/<?php echo $plugin_dir; ?>/array.php?key=city&part=");
			});
		...
		function delete_list(){
			...
			ids = ids.toString();
			var mySplitResult = ids.split("~");
			var r= confirm("Are you shure you want to delete..");
			if (r==true){
				$('#example-one').hide();
				$('#example-three').hide();
				$('.list-wraps').html('Progressing..........');
				$.ajax({
						url: "<?php echo get_bloginfo('url'); ?>/wp-content/plugins/amerisale-re/netriesdetail/delete_lising.php",
						data: 'mls='+mySplitResult[0]
						}).done(function(res) {
				if(res == 'There is no records in this Listings.'){
						alert('There is no records in this Listings.');
...
<h2> Manage Listings </h2><br clear="all" />
	<div class="contes">
						...
						global $wpdb;						
							...
							?>
							 ...
							 <table class="form-table">
								<tr valign="top">
									...
									<td><label>
										<?php //echo $html = generateSelect('city', $city); ?>
										<p id="auto">
											<input id="city" name="city" autocomplete="off" type="text" />
										</p>
									</label> </td>
								</tr>
							</table>
							...
							<table class="form-table">
								...
								<th scope="row">Select The County:</th>
									...
									<td><label>
										<p id="cntyauto">
											<?php //echo $html = generateSelect('country', $country); ?>
											<input id="country" autocomplete="off" name="country" type="text" />
										</p>	
									</label> </td>
								</tr>
							</table>
		...
		$sql = "SELECT A.agent_license_no,A.Name,A.agent_id,B.MLS,B.directions,B.city,B.county,B.agentlist,B.is_manual_edit,B.city,B.state,B.zipcode,B.county,B.street_num,B.street_type,B.street_name,B.expired_date,B.liststatus,B.sold,B.proptype
				FROM agents AS A
				LEFT JOIN ntreislist AS B
					ON A.agent_license_no=B.agentlist WHERE B.agentlist != '' AND $WHERE_0fficelist AND B.delete_status ='0' ";
					$sql = mysql_query($sql); 
		...
		<div id="example-two">
			...
			<button onclick="delete_list();" type="button">Delete Listing!</button>
					...
					<?php 
						...
						{ 
								if($a_mls['is_manual_edit'] == 1){
									...
									$addresss = $a_mls['directions']." ".$a_mls['city'].", TX ".$a_mls['zipcode']." ".$a_mls['county']." county";
								}else if($a_mls['is_manual_edit'] == 0 || $a_mls['is_manual_edit'] == 2){
									$addresss = $a_mls['street_num'].' '.$a_mls['street_name'].' '.$a_mls['street_type'].' '.$a_mls['city'].' '.$a_mls['state'].' '.$a_mls['zipcode'].' '.$a_mls['county'].' county';
								}else{
									$addresss = '';
								}
		...
		/*$sql = "SELECT * from ntreisimages WHERE n_id = 38804"; 
		$sqls = mysql_query($sql); 

		while($row = mysql_fetch_assoc($sqls)){
		$sql1 = "INSERT INTO ntreisimages (n_id, mlsnum,imagename,caption_text,recordListingID) 
				VALUES ('174527','" . $row['mlsnum'] . "','" . $row['imagename'] . "','" . $row['caption_text'] . "','" . $row['recordListingID'] . "')"; 
		$sqls1 = mysql_query($sql1);  
		}*/
